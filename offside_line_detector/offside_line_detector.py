...
class OffsideLineDetector:
    ...
    def detect_offside_line(self, soccer_video: Video):
        ...
        # get vanishing point
        vanishing_point, vanishing_point_segments, _ = self.vanishing_point_finder.find_vanishing_point(soccer_video)
        # find players
        players, _ = self.player_detector.detect_players(soccer_video)
        # track players
        players, _ = self.player_tracker.track_players(soccer_video, players)
        # classify players in teams
        players, _ = self.player_sorter.sort_players(soccer_video, players)
        # detect and attacking team
        players, _ = self.team_classifier.classify_teams(soccer_video, players)
        # detect orientation
        orientation, _ = self.orientation_detector.detect_orientation(soccer_video, vanishing_point)
    ...
    def detect_and_draw_offside_line(self, soccer_video: Video, video_data: [FrameData]):
        ...
        while True:
            ...
            Timer.start('offside_line')
            result = self.detect_offside_line(soccer_video)
            elapsed_time = Timer.stop('offside_line')
